...
- **Backends drive the user's own CLIs.** Claude: the installed `claude` CLI headless (`--print --output-format stream-json`), with the MCP registration passed per run via `--mcp-config`/`--strict-mcp-config`, built-ins restricted to WebSearch/WebFetch, text and extended-thinking deltas streamed token-level as ephemeral in-progress output, and the result payload's final text delivered once as the persisted answer (Codex forwards its `reasoning` items and interim agent messages the same way; its last agent message becomes the answer). Codex: the installed `codex` CLI (`exec --json`) under the `read-only` sandbox with live web search, MCP registered through per-run `--config` overrides — nothing is written to `~/.codex/config.toml`. OpenCode: the installed `opencode` CLI (`run --format json`), with the MCP registration and a built-in tool lockdown (everything but web research disabled) written per turn into the broker's `.opencode-workspace/opencode.json` — the bearer token rides an env var through opencode's `{env:}` substitution, so only the loopback URL is on disk, and its briefing travels as the workspace `AGENTS.md` (the same mechanism Codex uses); text parts stream as ephemeral in-progress output and the last one becomes the answer, with `--session` ids resuming multi-turn conversations. Since OpenCode fans out to whatever providers the user configured, the panel offers no model list for it — "Default" runs the model their opencode config selects. `agent/cli.mjs` finds the CLIs (native installs, PATH, or npm-global shims resolved to their real entries); a missing or signed-out CLI just reports itself unavailable in the panel.
