...
| Checklists, tables, autoformat shortcuts | `src/utils/richText.ts` |
| Math fields (KaTeX) and their editor | `src/utils/mathField.ts`, `src/components/MathEditor.tsx` |
...
Rich structures live inside that same HTML and route every change through the owning view's normal input-event serialize-and-commit path:

- `src/utils/richText.ts` owns checklists (`ul.checklist` + per-item `data-checked`, checkbox hit-testing, Enter fixups), tables (`table.text-table`, Tab/arrow cell navigation, the context menu's structural edits), links (URL normalization to http/https/mailto, the space-triggered autolink, unsafe-scheme stripping), the markdown-style autoformat conversions, and a self-healing normalize pass run before each serialize. The tile views intercept link clicks so the webview never navigates: a plain click opens `src/components/LinkPreview.tsx` — display text and destination editable in place against the anchor element itself, an Open action, and a preview — and Ctrl/Cmd+Click opens externally. The preview has two tiers. On desktop, the popup asks the shell to unfurl the page (`fetch_link_metadata` in `src-tauri/src/lib.rs`: a capped, timed native GET that parses Open Graph/Twitter meta tags — native fetches are not subject to browser CORS, which is how chat apps' servers do it) and renders the page's own card: og:image, title, description. When no card is available (web build, no metadata, fetch failure), it falls back to an inert live `<object>` embed whose section appears only for a proven successful load — an `<object>`, not an iframe, because iframes fire load for Chromium's error pages too while `<object>` fires error for refused/failed embeds, so sites that block framing (X-Frame-Options/CSP) never show preview chrome.
- `src/utils/mathField.ts` owns math fields: atomic `span.math-field` elements whose light DOM stays empty — the document persists only the TeX source in `data-tex`/`data-display`, and KaTeX renders into a shadow root that `innerHTML` serialization can never see, so no stripping pass exists. Views re-render math after any live-DOM rewrite; print/PDF export bakes static KaTeX markup with `renderMathInHtml` and inlines the collected KaTeX rules.
- `src/components/MathEditor.tsx` is the single math-editing popover per editor view, summoned via `openMathEditor` from the toolbar's insert button or a click on a field.
...
Inkjet is the built-in AI agent: it edits the currently open inktile from its panel with zero setup — in the desktop app, opening the panel is all it takes when Claude Code, Codex, and/or OpenCode is installed and signed in on the machine (design history in [AGENT_INTEGRATION_PLAN.md](AGENT_INTEGRATION_PLAN.md)). The browser build cannot spawn processes, so Inkjet is a desktop feature — there, the panel shows a single "desktop-only" note instead of attempting to connect.

The setup screen opens with a one-line intro (what Inkjet is, and that it connects automatically to the CLIs on this machine) and auto-detects providers: every provider is always listed with its status detail, but unavailable ones (not installed or not signed in) render greyed out and disabled; with none available, a short "no AI CLIs" note with a Check again button replaces the list entirely. The user picks a provider, picks a model from that provider's list (the broker owns the model catalog and ships it in its status message; "Default" defers to the CLI's own configuration), and presses **Start session**, which opens the chat view and resets any backend resume state so the conversation starts fresh. Conversations persist per document: the panel mirrors the transcript (with its provider/model) into local storage as it grows, so opening the panel on a document with a stored conversation skips setup and drops straight back into the chat — across panel closes, document switches, and app restarts — and the broker resumes the matching CLI conversation on the next turn. A stored chat whose provider has since become unavailable falls back to the setup screen. "Exit session" is the one gesture that ends a conversation: it returns to the setup screen and discards both the stored chat and the backends' resume state.

Chat view mechanics: the panel's left edge drags to resize (width persisted, 280–760px, UI-scale-aware); the composer grows with its content up to a cap instead of carrying a resize grip; the transcript hides the native scrollbar and is driven by `ElementScrollbar`, the vertical element-bound sibling of the workspace/row overlay scrollbars; and agent answers render through `InkjetMarkdown`, a dependency-free renderer that builds React elements (never raw HTML, so model output cannot inject markup) for the chat subset of markdown — paragraphs, headings, lists, quotes, code, bold/italic, and links. Bubbles stop short of the far edge (the side each hugs marks its sender) and animate in. In-progress output is visually separated from the final reply: everything the model says while the turn runs — reasoning plus the interim process notes between tool calls — streams as an ephemeral, visually distinct "thinking" bubble (`thinking` protocol messages, kept out of `transcript`), and only the turn's final answer (`answer` message, sent once when the backend's turn completes) replaces it as a persisted transcript card. All model text (thinking and answers) reveals as a fast typewriter — a few characters per frame, accelerating with the backlog — instead of snapping whole chunks in; entries that already typed render instantly when the panel remounts. The moving parts:
...
- **Backends drive the user's own CLIs.** Claude: the installed `claude` CLI headless (`--print --output-format stream-json`), with the MCP registration passed per run via `--mcp-config`/`--strict-mcp-config`, built-ins restricted to WebSearch/WebFetch, text and extended-thinking deltas streamed token-level as ephemeral in-progress output, and the result payload's final text delivered once as the persisted answer (Codex forwards its `reasoning` items and interim agent messages the same way; its last agent message becomes the answer). Codex: the installed `codex` CLI (`exec --json`) under the `read-only` sandbox with live web search, MCP registered through per-run `--config` overrides — nothing is written to `~/.codex/config.toml`. OpenCode: the installed `opencode` CLI (`run --format json`), with the MCP registration and a built-in tool lockdown (every file/exec tool denied through opencode's `permission` map — set on both global and the `build` agent that `run` uses, since agent config overrides global; the deprecated boolean `tools` map rides along only as a fallback for pre-permission builds — so the agent acts solely through the MCP tools, web research aside) written per turn into a per-process temp-dir `opencode.json` and pinned by absolute path through the `OPENCODE_CONFIG` env var — the config must not be *discovered*, because opencode resolves "the project" by walking up to the nearest `.git`, so a repo-nested cwd would make it adopt that repo, ignore our config, and edit the app's own source with file tools on; the absolute-path pin makes it cwd-proof, the same way `--mcp-config` and `--config`/`--sandbox` pin Claude and Codex. The run's actual cwd is a stable, empty, never-a-repo project dir under the app data dir, because opencode derives session identity from the project directory and `--session` resume across app runs needs it identical every time. The bearer token rides an env var through opencode's `{env:}` substitution, so only the loopback URL is on disk; the per-turn briefing is referenced by absolute path via the config's `instructions` key. Text parts stream as ephemeral in-progress output and the last one becomes the answer, with `--session` ids resuming multi-turn conversations. Since OpenCode fans out to whatever providers the user configured, the panel offers no model list for it — "Default" runs the model their opencode config selects. `agent/cli.mjs` finds the CLIs (native installs, PATH, or npm-global shims resolved to their real entries); a missing or signed-out CLI just reports itself unavailable in the panel.
...
- **Tool surface — full document control.** `read_document` returns everything (title, rows, text HTML, notes, versions with drafts + active index, per-stroke drawing summaries (id, tool, width, opacity, point count, bounds), alignment, and real pixel geometry: the document `pageWidth`, each tile's row height and rendered `widthPx`, and intrinsic image/video dimensions measured from the asset bytes and cached per asset); `read_drawing` returns a drawing page's full stroke points plus its canvas geometry; writes cover `set_title`, `append_text`/`edit_text`, `edit_notes` (back faces), `insert_page`/`delete_pages`/`arrange_pages`, `set_row_height`/`set_row_widths`/`set_vertical_align` (both sizing ops enforce the same limits as the drag gestures: heights clamp to 96–1600px with per-type content floors — media 120, versions 164, drawing 240 — and `set_row_height` reports the applied value; width splits reject columns under the 120px drag floor), `create_drawing`/`edit_drawing` (normalized 0..1 strokes, append or replace), `modify_strokes`/`delete_strokes` (stroke-level editing by id: translate, scale about an origin, restyle tool/width/opacity, remove), `insert_versions`/`edit_versions`/`convert_versions_to_text`, `create_image` (sanitized SVG), and `fetch_media`. Inserted image/video rows auto-size to the media's aspect ratio at full document width, so "contain" fills the tile instead of letterboxing. Deletion included: everything reverts with the turn's single undo. `fetch_media` sniffs the downloaded bytes (magic numbers) and labels assets by what they actually are — mislabeled server MIME types are corrected and text/HTML responses are rejected instead of landing as unplayable media.
- **Turn lock.** Sending a prompt calls `beginAgentTurn()`: one undo checkpoint is recorded, `agentTurn` renders the workspace read-only (`workspace--agent-locked`, non-editable text blocks, a fixed "Inkjet is printing" indicator with Stop, and a full-viewport scrim — `.inkjet-lock-scrim` — that dims and blocks every surface except the Inkjet panel, its toggle, and the indicator; wheel scrolling still reaches the document so the user can follow along), and `commit` rejects non-agent mutations. Agent ops run inside `runAgentEdit`, which suppresses per-op history — an entire turn reverts with a single Ctrl+Z. Ops apply under `flushSync`, so document state is current the moment each op acks; what the user sees is paced by the presentation queue below.
- **Presentation queue.** `src/agent/animations.ts` reveals committed agent edits sequentially, one op's visual effect after another: text types out (and deletes backward) at a fast typewriter pace in `TextBlockView`, drawing strokes draw point by point one stroke at a time in `DrawingCanvas` (removed strokes erase one by one, moved/restyled strokes glide into place), and agent-driven row sizing eases through CSS transitions scoped to `workspace--agent-locked` (user drag-resizes stay one-to-one). Animations are presentation-only — acks, the revision guard, undo, and autosave always see the committed state — so Stop or turn end just cancels the queue (an epoch bump stale-checks every runner) and the tile views snap to the real document. `prefers-reduced-motion` collapses every reveal to an instant snap. Tiles created after the document opened (any source, not just agent ops) mount with a brief fade (`page-row__cell--born`, tracked against the id set `PageStack` captured on mount). Reveal tasks also report their target element (`reportAgentFocus`), which powers the indicator's follow toggle: while armed, the panel centers the viewport on each reported element (smooth y-scroll), and any manual document scroll — wheel or page keys outside the panel — disarms it.
...
- **Re-reads only when needed.** The broker remembers each document's revision at the end of the last cleanly finished turn. When the next prompt arrives with the same revision (and the CLI conversation is resumable, so the model still remembers the document), the turn is briefed "unchanged — skip read_document"; otherwise "changed" or "first". The briefing is enforced, not just suggested: on changed/first turns the broker seeds the op channel with a sentinel revision, so any write attempted before `read_document` bounces off the guard. Stopped or failed turns and "Exit session" clear the memory, forcing a fresh read.
- **Failure handling.** Stop, backend errors, and a dying broker all end the turn the same way: the lock releases, streamed content stays (still one undo step), and the panel reports what happened. Leaving the editor unmounts the panel, which stops the turn and detaches; the broker stays warm and exits with the app. Per-document CLI session ids live in a small disk store (`agent/session-store.mjs`, under the app data dir), so multi-turn resume survives broker and app restarts; the same module provides the stable working directories that cwd-keyed CLI histories require (claude stores sessions per cwd "project", opencode derives session identity from the project dir; codex resumes by thread id regardless of cwd). A resume id that no longer resolves errors that one turn and is dropped with it, so the next prompt starts a fresh conversation instead of wedging the document.
